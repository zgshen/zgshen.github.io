...
- [0.1. Java 内存模型](#01-java-内存模型)
    - [0.1.1. 概念](#011-概念)
    - [0.1.2. JMM 关于同步的规定](#012-jmm-关于同步的规定)
    - [0.1.3. 主内存和工作内存](#013-主内存和工作内存)
    - [0.1.4. 并发编程三个特性](#014-并发编程三个特性)
    - [0.1.5. Happens-Before规则](#015-happens-before规则)
    - [0.1.6. Java 8种关于主内存和工作内存的交互协议操作](#016-java-8种关于主内存和工作内存的交互协议操作)
    - [0.1.7. volatile 禁止指令重排序，保证变量对所有线程的可见性](#017-volatile-禁止指令重排序保证变量对所有线程的可见性)
- [0.2. JVM](#02-jvm)
    - [0.2.1. JVM 内存模型](#021-jvm-内存模型)
    - [0.2.2. 运行时数据区](#022-运行时数据区)
        - [0.2.2.1. 虚拟机栈](#0221-虚拟机栈)
        - [0.2.2.2. 本地方法栈](#0222-本地方法栈)
        - [0.2.2.3. 程序计数器](#0223-程序计数器)
        - [0.2.2.4. 堆](#0224-堆)
        - [0.2.2.5. 方法区](#0225-方法区)
    - [0.2.3. 三种常量池](#023-三种常量池)
        - [0.2.3.1. 常量池](#0231-常量池)
        - [0.2.3.2. 运行时常量池](#0232-运行时常量池)
        - [0.2.3.3. 字符串常量池](#0233-字符串常量池)
- [0.3. 类加载机制](#03-类加载机制)
    - [0.3.1. 类加载过程](#031-类加载过程)
    - [0.3.2. 类加载器](#032-类加载器)
    - [0.3.3. 双亲委派机制](#033-双亲委派机制)
    - [0.3.4. 双亲委派机制如何打破](#034-双亲委派机制如何打破)
    - [0.3.5. 类初始化顺序](#035-类初始化顺序)
    - [0.3.6. Java的对象结构](#036-java的对象结构)
- [0.4. 内存分配与垃圾回收](#04-内存分配与垃圾回收)
    - [0.4.1. 内存分配策略](#041-内存分配策略)
    - [0.4.2. 垃圾回收判断](#042-垃圾回收判断)
    - [0.4.3. 垃圾回收算法](#043-垃圾回收算法)
    - [0.4.4. 垃圾回收器](#044-垃圾回收器)
    - [0.4.5. GC 回收机制](#045-gc-回收机制)
        - [0.4.5.1. 详细说明](#0451-详细说明)
        - [0.4.5.2. 为什么有两个 Survivor 区](#0452-为什么有两个-survivor-区)
        - [0.4.5.3. 相关参数](#0453-相关参数)
    - [0.4.6. Minor GC 和 Full GC](#046-minor-gc-和-full-gc)
    - [0.4.7. Full GC 触发条件](#047-full-gc-触发条件)
- [0.5. JVM 问题和调优](#05-jvm-问题和调优)
    - [0.5.1. 内存泄漏和内存溢出的区别](#051-内存泄漏和内存溢出的区别)
    - [0.5.2. OOM相关](#052-oom相关)
            - [0.5.2.0.1. 堆溢出](#05201-堆溢出)
            - [0.5.2.0.2. 栈内存溢出（-Xss）](#05202-栈内存溢出-xss)
            - [0.5.2.0.3. 方法区和运行时常量池溢出（-XX:PermSize -XX:MaxPermSize）](#05203-方法区和运行时常量池溢出-xxpermsize--xxmaxpermsize)
            - [0.5.2.0.4. 本机直接内存溢出（-MaxDirectMemorySize）](#05204-本机直接内存溢出-maxdirectmemorysize)
    - [0.5.3. OOM 常见原因](#053-oom-常见原因)
    - [0.5.4. 各类 OOM 和解决方法](#054-各类-oom-和解决方法)
    - [0.5.5. JVM 调优](#055-jvm-调优)
    - [0.5.6. 调优命令](#056-调优命令)
    - [0.5.7. 调优工具](#057-调优工具)
...
### 0.1.6. Java 8种关于主内存和工作内存的交互协议操作
...
### 0.1.7. volatile 禁止指令重排序，保证变量对所有线程的可见性
...
![img](https://github.com/zgshen/code-note/raw/master/doc/images/jvm-struct.png)
...
##### 0.5.2.0.3. 方法区和运行时常量池溢出（-XX:PermSize -XX:MaxPermSize）
...
##### 0.5.2.0.4. 本机直接内存溢出（-MaxDirectMemorySize）
